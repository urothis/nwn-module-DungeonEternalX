                                           ...
                                           int nBaseType3=BASE_ITEM_INVALID)
{
    ...
    // only happen once.
    CTG_InitContainer(oBaseCont);
    int nNumberOfItems = CTG_GetNumItemsInBaseContainer(oBaseCont);
    int nCounter = nNumberOfItems;
    int nMatch;
    int nRandom;
    // If there are no items in the container, no chance of finding a matching one!
    if ( nNumberOfItems == 0 ) return OBJECT_INVALID;
    int i; // start at zero
    string sVar;
    int nItemBaseType;
    // loop through one time, unless selection is made first
    while ( i < nNumberOfItems ) {
        ...
        {
            nRandom = Random(nCounter);
            if ( nRandom == 0 ) {
                // we found a matching item -- return it
                return CTG_GetTreasureItem(oBaseCont, i);
            }
            // count and save info when item has correct type
            else {
                --nCounter;
                SetLocalInt( oBaseCont, "Match_"+IntToString(nMatch), i );
                ++nMatch;
            }
        }
        ++i;
    ...
    nRandom = Random( nMatch );
    i = GetLocalInt( oBaseCont, "Match_"+IntToString(nRandom) );
    return CTG_GetTreasureItem(oBaseCont, i);
}
