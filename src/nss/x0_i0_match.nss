...
// Return TRUE if oItem is a Double-Sided Weapon
int MatchTwoSidedWeapon(object oItem);
...
int GetHasEffect(int nEffectType, object oTarget = OBJECT_SELF, int nMatchDuration = -1);
...
int GetHasEffect(int nEffectType, object oTarget = OBJECT_SELF, int nMatchDuration = -1)
{
    ...
    while (GetIsEffectValid(eCheck))
    {
        ...
        {
            if (nMatchDuration == -1) return TRUE;
            else if (GetEffectDurationType(eCheck) == nMatchDuration) return TRUE;
        }
