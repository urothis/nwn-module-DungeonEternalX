...
//End
    ...
    if(GetIsObjectValid(oTarget) && oTarget != GetAreaOfEffectCreator())
    {
        ...
        {
            SignalEvent(oTarget, EventSpellCastAt(OBJECT_SELF, GetEffectSpellId(eDark)));
        }
        else
        {
            SignalEvent(oTarget, EventSpellCastAt(OBJECT_SELF, GetEffectSpellId(eDark), FALSE));
        }
        // Creatures immune to the darkness spell are not affected.
        if ( ResistSpell(OBJECT_SELF,oTarget) != 2 && !GetHasSpellEffect(SPELL_TRUE_SEEING) && !GetHasSpellEffect(SPELL_DARKVISION))
        {
