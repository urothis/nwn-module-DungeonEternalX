/**
 *  $Id: zdlg_include_i.nss,v 1.3 2005/08/07 04:30:29 pspeed Exp $
 *
...
const int    DLG_CONVO_COUNT  = 20; // NUMBER OF DYNAMIC Z-CONVOS AVAILABLE (Not Using the Default conv)
...
int DLG_BASE_TOKEN = GetLocalInt(GetPCSpeaker(), DLG_CONVO_BASE); //4200;
...
object GetPcDlgSpeaker()
{
    ...
    if( oPC == OBJECT_INVALID )
        PrintString( "WARNING: Unable to retrieve a PC speaker." + GetName(OBJECT_SELF));
    return( oPC );
}
...
void SetDlgResponseList( string listId, object oListHolder )
{
    ...
    //SetLocalInt( oSpeaker, DLG_STATE, DLG_STATE_INIT );
...
{
     // Setup Starting Token is missing use default ZD token
    if (DLG_BASE_TOKEN==0) {
       SetLocalInt(oPC, DLG_CONVO_BASE, 4200);
       DLG_BASE_TOKEN = 4200;
    }
...
void EndDlg()
{
    ...
    if (GetLocalInt(oSpeaker, DLG_CONVO_SLOT)) { // FREE UP SLOT IN OUR CONVO LIST IF TAKING UP ONE
      ReplaceObjectElement(GetLocalInt(oSpeaker, DLG_CONVO_SLOT), OBJECT_INVALID, DLG_CONVO_LIST, GetModule());
      DeleteLocalInt(oSpeaker, DLG_CONVO_SLOT);
    }
}
...
int _SetupDlgResponse( int num, object oSpeaker )
{
    ...
    if( (hasNext || hasPrev || hasEnd) && num >= 20 )
        {
        if( hasNext && num == 20 )
            {
            SetCustomToken( DLG_BASE_TOKEN + 21, GetRGB(15,5,1) + "[Next]" );
            return( TRUE );
            }
        if( hasPrev && num == 21 )
            {
            SetCustomToken( DLG_BASE_TOKEN + 22, GetRGB(15,5,1) + "[Previous]" );
            return( TRUE );
            }
        if( hasEnd && num == 22 )
            {
            SetCustomToken( DLG_BASE_TOKEN + 23, GetRGB(15,5,1) + "[End]" );
            return( TRUE );
            }
        return( FALSE );
        }
    ...
    switch( selection )
        ...
        case 20:
            if( _HasDlgNext( oSpeaker ) )
                {
                // Next page
                _SetDlgFirstResponse( oSpeaker, first + 20 );
                return;
                }
            break;
        case 21:
...
void OpenNextDlg( object oPC, object oObjectToConverseWith, string dlgHandler, int bPrivate = FALSE, int bPlayHello = TRUE, int bZoom = TRUE ) {
   ...
   if (DLG_CONVO_NOFAIL) { // USE A ROTATING CONVO COUNTER SO EVERY 10 CONVOS START OVER
      ...
      i = GetElementCount(DLG_CONVO_LIST, oModule);
      if (i==0) { // INITIALIZE THE LIST
         for (i=1; i<=DLG_CONVO_COUNT; i++) {
            AddObjectElement(OBJECT_INVALID, DLG_CONVO_LIST, oModule); // PLACEHOLDERS
         }
      }
      for (i=1; i<=DLG_CONVO_COUNT; i++) {   // FIND AN OPEN CONVO
          oConvo = GetObjectElement(i-1, DLG_CONVO_LIST, oModule);
          ...
          if (!IsInConversation(oConvo)) { // THE LIST THINKS THIS SLOT IS TAKEN, BUT IT'S OBJECT SAYS NOT SO, CLEAR IT AND USE THIS SLOT
            ReplaceObjectElement(i-1, OBJECT_INVALID, DLG_CONVO_LIST, GetModule());
            break;
         }
      }
      ...
      ReplaceObjectElement(i-1, oPC, DLG_CONVO_LIST, oModule);
      SetLocalInt(oPC, DLG_CONVO_SLOT, i);
   }
   SetLocalInt(oPC, DLG_CONVO_BASE, 4200 + i * 100);
   string dlg = "zdlg_convo_" + IntToString(i);
   // Setup the conversation
   SetLocalString(oPC, DLG_CURRENT_HANDLER, dlgHandler);
