...
void main()
{
    ...
    while(GetIsObjectValid(oTarget) && !bBreak)
    {
        if(spellsIsTarget(oTarget,SPELL_TARGET_SELECTIVEHOSTILE,OBJECT_SELF) && oTarget != OBJECT_SELF)
        {
            ...
            {
                if((!MySavingThrow(SAVING_THROW_WILL, oTarget, nDC, SAVING_THROW_TYPE_MIND_SPELLS, OBJECT_SELF, fDelay)) && (!GetHasEffect(IMMUNITY_TYPE_DOMINATE,oTarget)))
                {
                    //--------------------------------------------------------------------------
