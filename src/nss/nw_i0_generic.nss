       ...
       || GetHasFeat(FEAT_HIDE_IN_PLAIN_SIGHT, oSelf) == TRUE)
    {
        ...
        if(InvisibleTrue(oSelf) == FALSE)
        {
           ...
           //     }
            ...
            if(!GetIsObjectValid(oSeeMe))
            {
                int nDiff = GetCombatDifficulty(oSelf);
...
int chooseTactics(object oIntruder)
{
    ...
    // if (GetDistanceToObject(oIntruder) > 3.0) nMagic = nMagic + 15;

    // * Dec 18 2002: Add your level to your magic rating
    nMagic += 15 + GetHitDice(OBJECT_SELF);
   ...
   // **************************************
    ...
    // changed by ezramun, just build your mobs right, dammit
    ...
    // disabled by ezramun, to expensive
    ...
    if ((nOffense<= 50) && (nMagic > 50) && (nCompassion <=50))
    {
        ...
        /* Dec 19 2002: DISABLED BY EZRAMUN
    ...
    if (nMagic > 50)
    {
        ...
        //if (TalentUseTurning() == TRUE) return 99; EZRAMUN, expensive crap
    ...
    // if (TryKiDamage(oIntruder) == TRUE) return 99; // EZRAMUN
    // if (TalentSneakAttack() == TRUE) return 99; // EZRAMUN
