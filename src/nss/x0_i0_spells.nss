...
void DoMissileStorm(int nD6Dice, int nCap, int nSpell, int nMIRV = VFX_IMP_MIRV, int nVIS = VFX_IMP_MAGBLUE, int nDAMAGETYPE = DAMAGE_TYPE_MAGICAL, int nONEHIT = FALSE, int nReflexSave = FALSE, int nDC = 0, int bHasNekrosis = FALSE);
...
void DoCamoflage(object oTarget)
{
    ...
    ApplyEffectToObject(DURATION_TYPE_INSTANT, eVis, oTarget);
    ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eHide, oTarget, TurnsToSeconds(nDuration));
}
//::///////////////////////////////////////////////
//:: DoSpikeGrowthEffect
//:: Copyright (c) 2001 Bioware Corp.
            ...
            if (!MySavingThrow(SAVING_THROW_FORT, oTarget, nSaveDC))
            {
                ...
                if (bIsPC == TRUE)
                {
                    if (bShowPopup == TRUE)
                    {
                        ...
                        ApplyEffectToObject(DURATION_TYPE_TEMPORARY, ePetrify, oTarget, fDifficulty);
                }
                else
                {
                    ApplyEffectToObject(DURATION_TYPE_PERMANENT, ePetrify, oTarget);
