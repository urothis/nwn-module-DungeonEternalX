...
{   //SpawnScriptDebugger();
    ...
    if(!GetAssociateState(NW_ASC_IS_BUSY))
    {
        ...
        if(GetIsObjectValid(oMaster) && nCurrentAction != ACTION_FOLLOW && nCurrentAction != ACTION_DISABLETRAP && nCurrentAction != ACTION_OPENLOCK && nCurrentAction != ACTION_REST && nCurrentAction != ACTION_ATTACKOBJECT)
        {
            if (!GetIsObjectValid(GetAttackTarget()) && !GetIsObjectValid(GetAttemptedSpellTarget()) && !GetIsObjectValid(GetAttemptedAttackTarget()))
            {
