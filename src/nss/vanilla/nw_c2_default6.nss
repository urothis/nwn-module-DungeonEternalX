    ...
    else
    {
        ...
        else if (!GetIsFighting(OBJECT_SELF))
        {
            ...
            else
            {
                ...
                else if (GetHitDice(OBJECT_SELF) >= 60) // added by ezramun
                {
                    object oMaster = GetMaster(oDamager);
                    if (GetIsObjectValid(oMaster))
                    {
                        if (GetObjectSeen(oMaster)) oDamager = oMaster;
                    }
        ...
        else
        {
            ...
            if (!GetIsObjectValid(oTarget) || (oTarget != oDamager && GetTotalDamageDealt() > GetMaxHitPoints(OBJECT_SELF)/4))
            {
                // Switch targets
                DetermineCombatRound(oDamager);
            }
