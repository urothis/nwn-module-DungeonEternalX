...
object GetNearestPerceivedEnemy(object oSource=OBJECT_SELF, int nNth = 1, int nCriteriaType=1000, int nCriteriaValue=1000)
{
    ...
    // never use heard enemy, it makes the source go hunt at wrong places

    // added by ezramun
    if (GetHitDice(oSource) >= 60)
    {
        object oMaster = GetMaster(oTarget);
        if (GetIsObjectValid(oMaster))
        {
            if (GetObjectSeen(oMaster, oSource)) return oMaster;
        }
    }
    return oTarget;
}
...
int CheckFriendlyFireOnTarget(object oTarget, float fDistance = 5.0)
{
    ...
    //SpeakString("CheckFriendlyFireOnTarget: " + IntToString(nCnt));
    return nCnt;
}
...
int CheckEnemyGroupingOnTarget(object oTarget, float fDistance = 5.0)
{
    ...
    //SpeakString("CheckEnemyGroupingOnTarget: " + IntToString(nCnt));
    return nCnt;
}
...
object FindSingleRangedTarget()
{
    ...
    //SpeakString("FindSingleRangedTarget: " + IntToString(nCnt));
    return oTarget;
}
