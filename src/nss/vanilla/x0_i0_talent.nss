...
int TalentMeleeAttacked(object oIntruder = OBJECT_INVALID)
{
    ...
    talent tUse;
    int nMelee = d2(); // GetNumberOfMeleeAttackers(); // changed by ezramun
    ...
    */
    ...
    if(nMelee == 1)
    {
        ...
        if(GetIsTalentValid(tUse))
        {
            ...
            {
                //MyPrintString("TalentMeleeAttacked genericAttemptHarmfulRanged Successful Exit");
                return TRUE;
            }
        }
...
int TalentRangedAttackers(object oIntruder = OBJECT_INVALID)
{
    ...
    {
        // Ezramun:
        int nFriendlyFire = 0; //CheckFriendlyFireOnTarget(oTarget)
        int nEnemyGrouping = d2() - 1; //CheckEnemyGroupingOnTarget(oTarget)
        if(nFriendlyFire > 0)
        {
            tUse = GetCreatureTalent(TALENT_CATEGORY_HARMFUL_AREAEFFECT_DISCRIMINANT, nCR);
            if(GetIsTalentValid(tUse))
...
int TalentRangedEnemies(object oIntruder = OBJECT_INVALID)
{
    ...
    if(GetIsObjectValid(oTarget))
    {
        // Ezramun:
        int nFriendlyFire = 0; //CheckFriendlyFireOnTarget(oTarget)
        int nEnemy = d2() - 1; //CheckEnemyGroupingOnTarget(oTarget)

        if(nFriendlyFire > 0 &&  nEnemy > 0)
        {
...
int TalentSpellAttack(object oIntruder)
{
    ...
    if (GetIsTalentValid(tUse) == FALSE)
    {
        ...
        int nFriends = 0; //CheckFriendlyFireOnTarget(oIntruder) ; // ezramun
        if (nFriends <= 1)
        {
...
//     nRacialType != RACIAL_TYPE_CONSTRUCT ||
       ...
       nRacialType != RACIAL_TYPE_ANIMAL)
    {
        ...
        if( (nCurrent < nBase) || (bForce == TRUE) )
        {
            ...
            /*else  // ezramun: skip this, DeX not using it
            {
...
/*    if (nForce == TRUE)
    ...
    */
    ...
    if (GetAILevel() > 1) // added by ezramun
    {
