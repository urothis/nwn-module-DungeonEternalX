...
object bkAcquireTarget()
{
    ...
    if (GetIsObjectValid(oLastTarget) == TRUE && !GetAssociateState(NW_ASC_MODE_DYING, oLastTarget))
    {
        return oLastTarget;
    } else {
...
object ChooseNewTarget()
{
    ...
    return GetNearestPerceivedEnemy();
}
    ...
    if (GetAILevel() > 1)
    {
        ...
        if (nTargetRacialType == RACIAL_TYPE_UNDEAD)
        {
            ...
            if (MatchInflictTouchAttack(iId) || MatchMindAffectingSpells(iId))
            {
                ...
                if (GetIsTalentValid(tTemp) && GetIdFromTalent(tTemp) == SPELL_HEAL && GetChallengeRating(oTarget) > 8.0)
                {
                    tFinal = tTemp;
                    iAmDone = TRUE;
                }
        ...
        if (iId == SPELL_POWER_WORD_KILL && !iAmDone)
        {
            if (GetCurrentHitPoints(oTarget) > 100)
            {
                ...
                if(GetIsTalentValid(tUse)) nNotValid = FALSE;
                else DecrementRemainingSpellUses(OBJECT_SELF, SPELL_POWER_WORD_KILL);
            }
        }
