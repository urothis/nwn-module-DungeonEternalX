...
const int NW_I0_SPELLS_MAX_BREACH = 32;
...
void spellsCure(int nDamage, int nMaxExtraDamage, int nMaximized, int vfx_impactHurt, int vfx_impactHeal, int nSpellID)
{
    ...
    //  Adds an additional Max of 20 pts for healing domain.
    //  if (GetHasFeat(FEAT_HEALING_DOMAIN_POWER, OBJECT_SELF)) nDamage += d2(nCasterLevel/10);
...
void RemoveEffectsFromSpell(object oTarget, int SpellID)
{
    if (GetHasSpellEffect(SpellID, oTarget))
    {
...
int MyResistSpell(object oCaster, object oTarget, float fDelay = 0.0) {
    ...
    effect eVisSR = EffectVisualEffect(VFX_IMP_MAGIC_RESISTANCE_USE);
    effect eVisGlobe = EffectVisualEffect(VFX_IMP_GLOBE_USE);
    effect eVisMantle = EffectVisualEffect(VFX_IMP_SPELL_MANTLE_USE);
    ...
    if(nResist == 1)
    {
        ...
        if      (GetHasFeat(FEAT_EPIC_SPELL_PENETRATION,    oCaster)) nRollBonus = 6;
        else if (GetHasFeat(FEAT_GREATER_SPELL_PENETRATION, oCaster)) nRollBonus = 4;
        else if (GetHasFeat(FEAT_SPELL_PENETRATION,         oCaster)) nRollBonus = 2;

        if ((nCasterLvl + nRoll + nRollBonus >= nSR || nRoll == 20) && nRoll != 1)
        {
            sMsg = "Penetrated: ";
            nResist = 0;
            DelayCommand(fDelay, ApplyEffectToObject(DURATION_TYPE_INSTANT, eVisSR, oTarget));
        }
        else
        {
            sMsg = "Resisted: ";
        ...
        sMsg += IntToString(nCasterLvl) + " + " + IntToString(nRoll) + " + " + IntToString(nRollBonus) + " = " + IntToString(nCasterLvl + nRoll + nRollBonus)
              + " vs SR " + IntToString(nSR);
    ...
    {
        DelayCommand(fDelay, ApplyEffectToObject(DURATION_TYPE_INSTANT, eVisGlobe, oTarget));
    }
    //Spell Mantle
    else if(nResist == 3)
    {
        ...
        DelayCommand(fDelay, ApplyEffectToObject(DURATION_TYPE_INSTANT, eVisMantle, oTarget));
    }
...
effect GetScaledEffect(effect eStandard, object oTarget)
{
    ...
    if (GetIsPC(oTarget) || (GetIsObjectValid(oMaster) && GetIsPC(oMaster)))
    {
        ...
        {
            eNew = EffectLinkEffects(EffectDazed(), EffectCutsceneParalyze());
            return eNew;
        }
        // DeX not using all this crap
        /*else if(GetEffectType(eStandard) == EFFECT_TYPE_FRIGHTENED && nDiff == GAME_DIFFICULTY_VERY_EASY)
        {
        ...
        if(!GetIsReactionTypeFriendly(oTarget))
        {
            ...
            if(o2ndTarget != oTarget)
            {
                ...
                if (nDamage > 0)
                {
                    ...
                    DelayCommand(0.1, ApplyEffectToObject(DURATION_TYPE_INSTANT, eDam, o2ndTarget));
