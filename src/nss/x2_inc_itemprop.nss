...
const int    X2_IP_MAX_ITEM_PROPERTIES = 5;
...
object IPGetTargetedOrEquippedArmor(int bAllowShields = FALSE)
{
  ...
  if(GetIsObjectValid(oTarget) && GetObjectType(oTarget) == OBJECT_TYPE_ITEM)
  {
    ...
    {
        if ((bAllowShields && GetLocalString(oTarget, "OWNER") == "") && (GetBaseItemType(oTarget) == BASE_ITEM_LARGESHIELD ||
        ...
        {
          if (GetLocalString(oTarget, "OWNER") != "") FloatingTextStringOnCreature("Can not be cast on Epic Items", oTarget);
          return OBJECT_INVALID;
        }
    }
  ...
  {
    if (GetLocalString(oArmor1, "OWNER") != "")
    {
      FloatingTextStringOnCreature("Can not be cast on Epic Items", oTarget);
      return OBJECT_INVALID;
    }
    return oArmor1;
  }
