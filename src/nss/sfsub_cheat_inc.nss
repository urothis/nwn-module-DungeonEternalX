...
//::///////////////////////////////////////////////
//:: Scarface's Anti Cheat Include Script
//:: sf_anticheat
//:://////////////////////////////////////////////
...
int SF_GetHasLegalHitpoints(object oPC)
{
    int nHD = 0;
...
int SF_GetHasLegalSkillPoints(object oPC)
{
...
void SF_HackerAction(object oPC, int nRunFromDefault = FALSE)
{
    string sHackReason = GetDeity(oPC);
    ...
    // DelayCommand(0.1, AssignCommand(oPC, JumpToObject(GetWaypointByTag("WP_HACKER"))));
    ...
    DelayCommand(4.0, ActivatePortal(oPC, LETO_PORTAL_IP_ADDRESS, LETO_PORAL_SERVER_PASSWORD, "", TRUE));
    DelayCommand(8.0, BootPC(oPC));
    */
    DelayCommand(9.3, NWNX_Administration_DeletePlayerCharacter(oPC));
}

string SF_GetIsValidName(string sName)
{
    string sReturn;
    ...
    /*
    else if (FindSubString(sAccount, "~") != -1 ||
             FindSubString(sAccount, "")  != -1)
    {
        sReturn = "Account Name contain invalid character (tilde or seperator)";
    }
    */
...
int SF_GetIsCharacterLegal(object oPC, string sName)
{
    ...
    if (!SF_GetHasLegalHitpoints(oPC))
    {
        nIllegal ++;
        sIllegal += "\n'Illegal Hitpoints'";
    }
    if (!SF_GetHasLegalSkillPoints(oPC))
    {
        nIllegal ++;
        sIllegal += "\n'Illegal Skill Points'";
    }
    string sValidName = SF_GetIsValidName(sName);
    ...
    else // name is valid, check unique
    {
        ...
        if (NWNX_SQL_ReadyToReadNextRow()) // Name created
        {
    ...
    if (nIllegal)
    {
        ...
        SetDeity(oPC, sIllegal);
        SetLocalString(oPC, "BANNED", "1"); // ez: this stops executing the OnClientExit
        DelayCommand(1.0, SF_HackerAction(oPC));
        return FALSE;
    }
    return TRUE;
}
