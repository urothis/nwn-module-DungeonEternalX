...
void RunInfestImpact(object oTarget, object oCaster, int nSaveDC, int nMetaMagic, int nPureBonus);
...
void main() {
   ...
   if(MyResistSpell(OBJECT_SELF, oTarget, fDelay) == 0) {
      ...
      SetLocalInt(oTarget,"XP2_L_SPELL_SAVE_DC_" + IntToString (SPELL_INFESTATION_OF_MAGGOTS), nPureDC);
      DelayCommand(fDelay+0.1f, RunInfestImpact(oTarget, OBJECT_SELF, nPureDC, nMetaMagic, nPureBonus));
   }
}
...
void RunInfestImpact(object oTarget, object oCaster, int nSaveDC, int nMetaMagic, int nPureBonus) {
   ...
   if (!GetIsDead(oTarget)) {
       if (!MySavingThrow(SAVING_THROW_FORT,oTarget,nSaveDC,SAVING_THROW_TYPE_DISEASE,OBJECT_SELF)) {
         ...
         int   nDamage = GetMax(nPureBonus/2, d4(1));
         ...
         else
         {
             ...
             DelayCommand(6.0, RunInfestImpact(oTarget,oCaster,nSaveDC, nMetaMagic, nPureBonus));
         }
